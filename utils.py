...
def load_triples(file_path, reverse=True):
    ...
    triples = np.array(
        [line.replace("\n", "").split("\t") for line in triples1 + triples2]
    ).astype(np.int64)
...
def load_aligned_pair(file_path, ratio=0.3):
    ...
    if os.path.exists(file_path + "ref_pairs"):
        ref_file = "ref_pairs"
        sup_file = "sup_pairs"
    else:
        ref_file = "ref_ent_ids"
        sup_file = "sup_ent_ids"

    if sup_file not in os.listdir(file_path):
        with open(file_path + ref_file) as f:
            aligned = f.readlines()
    else:
        with open(file_path + ref_file) as f:
            ref = f.readlines()
        with open(file_path + sup_file) as f:
            sup = f.readlines()
        aligned = ref + sup

    aligned = np.array([line.replace("\n", "").split("\t") for line in aligned]).astype(
        np.int64
    )
